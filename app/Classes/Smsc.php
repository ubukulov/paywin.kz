...
class Smsc
{
    ...
    protected $SMTP_FROM = "api@smsc.kz";     // e-mail адрес отправителя
    ...
    public function send_sms($phones, $message, $translit = 0, $time = 0, $id = 0, $format = 0, $sender = "SMSC.KZ", $query = "", $files = array())
    {
    ...
    {
        return mail("send@send.smsc.kz", "", $this->SMSC_LOGIN . ":" . $this->SMSC_PASSWORD . ":$id:$time:$translit,$format,$sender:$phones:$message", "From: " . $this->SMTP_FROM . "\nContent-Type: text/plain; charset=" . $this->SMSC_CHARSET . "\n");
    }
    ...
    {
        $url = $_url = ($this->SMSC_HTTPS ? "https" : "http") . "://smsc.kz/sys/$cmd.php?login=" . urlencode($this->SMSC_LOGIN) . "&psw=" . urlencode($this->SMSC_PASSWORD) . "&fmt=1&charset=" . $this->SMSC_CHARSET . "&" . $arg;
