        ...
        <div class="partdescr__main px-4 pt-6">
            ...
            <div class="flex flex-col gap-6 mb-8">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-xl font-bold text-gray-900 leading-tight">{{ $partnerProfile->company }}</h1>
                        <p class="text-sm text-gray-500">{{ $partnerProfile->category->title }}</p>
                    </div>
                </div>
                        ...
                        @foreach($partner->shares as $share)
                            <div class="swiper-slide !w-auto">
                                <div class="bg-orange-50 border border-orange-100 rounded-2xl p-4 min-w-[200px] h-full flex flex-col justify-between">
                                    ...
                                    <div class="text-[10px] font-bold text-orange-400 uppercase mb-2">при заказе от {{ number_format($share->data['from_order'], 0, '.', ' ') }} ₸</div>
                                    @endif
...
{{--                                        <img src="/images/partner-description/slider-elem.svg" alt="" class="">--}}
            ...
            <div class="mb-8">
                ...
                <div class="flex gap-3">
                    @if(!empty($partnerProfile->vk))
                        <a href="{{ $partnerProfile->vk }}" target="_blank" class="w-12 h-12 bg-gray-50 flex items-center justify-center rounded-xl hover:bg-blue-50 transition-colors shadow-sm">
                            <img src="/images/partner-description/vk-icon.svg" alt="vk" class="w-6 h-6">
                        </a>
                    @endif
                    @if(!empty($partnerProfile->telegram))
                        <a href="{{ $partnerProfile->telegram }}" target="_blank" class="w-12 h-12 bg-gray-50 flex items-center justify-center rounded-xl hover:bg-blue-50 transition-colors shadow-sm">
                            <img src="/images/partner-description/telegram-icon.svg" alt="telegram" class="w-6 h-6">
                        </a>
                    @endif
                    @if(!empty($partnerProfile->instagram))
                        <a href="{{ $partnerProfile->instagram }}" target="_blank" class="w-12 h-12 bg-gray-50 flex items-center justify-center rounded-xl hover:bg-pink-50 transition-colors shadow-sm">
